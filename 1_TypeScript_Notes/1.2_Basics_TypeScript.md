...
Now what if I wrap ```myBoolean```with a value of ```true``` with ```" "```.

You'll get a error obviously,

You just turned the value of ```true```, a boolean type to a string. If you wanted this variable to take a sting, go back and reassign the *Value Type* as string and not boolean, or even better assign ```any``` so that now we can assign any *Value Type*.
